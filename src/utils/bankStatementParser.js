...
export function detectBankStatement(text) {
    ...
    if (
        (t.includes("BANCO GALICIA") || t.includes("BANCOGALICIA") || t.includes("GALICIA")) &&
        t.includes("RESUMEN DE CUENTA CORRIENTE")
    ) {
        return {
            banco: "Galicia",
    ...
    if (
        t.includes("RESUMEN DE CUENTA CORRIENTE EN PESOS") &&
        t.includes("DATOS DE LA CUENTA") &&
        t.includes("CBU") &&
        t.includes("FECHA DESCRIPCION ORIGEN CREDITO DEBITO SALDO")
    ) {
        return {
            banco: "Galicia",
            tipoDocumento: "cuenta_corriente",
...
function parseGaliciaCuenta(text) {
    ...
    for (const line of lines) {
        ...
        const oneLineMatch = line.match(oneLineMovementRegex);
        if (oneLineMatch) {
            flush();

            const [, fechaRaw, descripcionRaw, movimientoRaw, saldoRaw] = oneLineMatch;
        ...
        const dateStartMatch = line.match(dateStartRegex);
        if (dateStartMatch) {
            flush();

            const [, fechaRaw, descripcionInicial] = dateStartMatch;

            current = {
                fecha: fechaRaw,
                descriptionLines: [descripcionInicial],
                amounts: [],
            };
        ...
        if (moneyTokens.length >= 2 && tokens.length === moneyTokens.length) {
            current.amounts.push(...moneyTokens);
            continue;
        }

        if (moneyTokens.length >= 2) {
            const descriptionTokens = tokens.filter((token) => !isMoneyToken(token));

            if (descriptionTokens.length) {
                current.descriptionLines.push(descriptionTokens.join(" "));
            }
        ...
        if (moneyTokens.length === 1 && tokens.length === 1) {
            current.amounts.push(moneyTokens[0]);
            continue;
        }
